...
preprocess()
{
	...
	opt -load ${HOME}/build/lib/SideEffect/libSideEffectPass.so -sideeffect-analysis -strFuncName ConvertMIDI -numLoopHeader 598 ${WORKING}/m2rtttl.opt.id > /dev/null
...
#opt -load ${HOME}/build/lib/Constraints/libConstraintsPass.so -collect-constraints -strFuncName ConvertMIDI -numLoopHeader 598 -strInstFile SD.inst -strSTName struct.MIDI_MSG ${WORKING}/m2rtttl.opt.id > /dev/null
